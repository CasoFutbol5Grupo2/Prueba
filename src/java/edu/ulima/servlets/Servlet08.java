 ...
 * @author D2042
 */
@WebServlet(name = "Servlet08", urlPatterns = {"/s08"})
public class Servlet08 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(eleccion.equalsIgnoreCase("e1")){
            String organizador = request.getParameter("organizador");
            lista = g.buscarPartidosPorOrganizador(organizador);
        } else if(eleccion.equalsIgnoreCase("e2")){
            String fecha = request.getParameter("fechaBusq");
            lista = g.buscarPartidosPorFecha(fecha);
        } else {
            lista = g.verPartidos();
        }
        ses.setAttribute("listaBuscada", lista);
